...
root = Tk() # i'm adding the GUI initialization
root.geometry("365x245")

lower_bound = float (sys.argv[1])# Im changing upper bound and lowerbound to  user input in text box
...
#if "phase" in sys.argv: #replacing sys argument coditions with checks for conditions in widgets
    if CheckVar1.get() == 1:
...
# all the GUI function calls are here
Calibration = Button(root, text="Calibrate", fg="Black", command = Calibrate).grid(row = 0, column = 0)
GraphDisplay = Button(root, text="Generate Plot", fg="Black",command = GraphPlot).grid(row = 0, column = 0)
